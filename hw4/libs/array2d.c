...
Array2d Array2d_get_array_data(json_t *array_data, int num_rows, int num_cols) {
	Array2d array = (Array2d)malloc(sizeof(Array2d_s));
        if(array == NULL) {
                printf("err out of memory");
                return NULL;
        ...
        for(int i = 0; i < num_rows; i++) {
                for(int j = 0; j < num_cols; j++){
                        int offset = i * num_cols + j;
                        json_t *element = json_array_get(array_data, (size_t)offset);
			int x = json_integer_value(element);
                        *(array->pData + offset) = (Array2d_value_t)x;
                }
        }
...
Array2d Array2d_copy(Array2d original) {	
	int num_rows = original->rows;
	int num_cols = original->cols;

	Array2d copy = (Array2d)malloc(sizeof(Array2d_value_t) * num_rows * num_cols);
	copy->rows = num_rows;
	copy->cols = num_cols;
	
	for (int i = 0; i < num_rows; i++) {
		for (int j = 0; j < num_cols; j++) {
			int val = Array2d_get(original, i, j);
			Array2d_set(copy, i, j, val);
		}
	}
	return copy;
}
